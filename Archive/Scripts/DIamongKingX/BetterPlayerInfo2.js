...
class InfoMenu extends Menu {
    ...
    createInfoElement(name, referenceName, activationCondition = () => { return true }, value, isVisible = true, index = 0) {
        const infoElement = createElement("p",
            ...
            },
            {
                ...
                index,
                setValue(value) {
                    return this.querySelector("strong").textContent = value ?? null
                }
            }).appendTo(this.body)
    ...
    parseUpdatedAttributes() {
        this.updatedAttributes.forEach(attribute => {
            ...
            this.createInfoElement(attribute.name, attribute.referenceName, attribute.activationCondition, attribute.value, attribute.isVisible, attribute.index)
        })
    }
...
function script() {
    ...
    defineScriptElement("entityFollower", "div", { id: "entityFollower" }, {
        ...
        update(time) {
            ...
            // Otherwise idle with slower speed while moving around to random destinations from the center of the screen
                ...
                this.setDestination(innerWidth / 2 + 220 * (-1 + Math.random() * 2), innerHeight / 2 + 220 * (-1 + Math.random() * 2))
            }
    ...
    addEventListener("mousedown", (event) => {
        if (main.scriptElements.mainMenuWrapper.contains(event.target) || !main.scriptElements.mainMenuWrapper.open || main.menu.pinned) {
    ...
    const mainInfoMenu = new InfoMenu("infoMenu", [
        ...
        { name: "Wood", referenceName: "wood" },
        { name: "Token", referenceName: "token" },
        { name: "Stone", referenceName: "stone" },
        {
            name: "Wave", value: (player) => {
                return player.targetTick.wave ?? "Not Found"
            }
        },
        { name: "Gold", referenceName: "gold" },
        { name: "Score", referenceName: "score" },
    ...
    mainInfoMenu.defineState(
        ...
        new InfoMenu("infoMenuStatePet", [
            ...
            { name: "Experience", referenceName: "experience" },
            { name: "Tier", referenceName: "tier" },
            {
                name: "Damage",
