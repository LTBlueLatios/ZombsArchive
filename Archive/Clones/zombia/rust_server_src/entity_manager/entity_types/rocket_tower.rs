...
const PROJECTILE_LOW_SPEED: [u8; TIERS] = [4, 4, 4, 5, 5, 6, 6, 8];

#[derive(Clone, Debug)]
pub struct RocketTower {
    pub ranged_building: AttackingBuilding,
    pub attacking_side: bool
}

impl BuildingTrait for RocketTower {
    fn new(uid: u16, position: super::generic_entity::Position, rotation: u16) -> Self {
        let mut entity = RocketTower {
            attacking_side: false,
            ranged_building: AttackingBuilding {
                aiming_yaw: 0,
                firing_tick: 0,
                range_cells: Vec::new(),
                base_building: Building::new(uid, position, rotation, "RocketTower".to_owned()),
...
impl AttackingBuildingTrait for RocketTower {
    ...
    fn fire_ranged(&self, enemy_uid: u16) {
        ...
        let mut projectile_position = self.ranged_building.base_building.generic_entity.position.clone();

        // Offset the projectile to spawn on top of the turret sprite
        let x_offset = (angle as f32 * PI / 180.0).sin() * 48.0;
        let y_offset = -(angle as f32 * PI / 180.0).cos() * 48.0;
        ...
        let attacking_direction: f32;

        if self.attacking_side == true {
            self.set_property("attacking_side", Box::new(false));

            attacking_direction = (((angle as i32 - 90) % 360 + 360) % 360) as f32;
        } else {
            self.set_property("attacking_side", Box::new(true));

            attacking_direction = (((angle as i32 + 90) % 360 + 360) % 360) as f32;
        }

        let x_offset = (attacking_direction * PI / 180.0).sin() * 16.0;
        let y_offset = -(attacking_direction * PI / 180.0).cos() * 16.0;
        ...
        let projectile_max_speed = building_info.projectile_speed[tier_index_usize] as f32;
        let projectile_low_speed = PROJECTILE_LOW_SPEED[tier_index_usize] as f32;

        let projectile_velocity= vector![
            (angle as f32 * PI / 180.0).sin() * projectile_max_speed,
            -(angle as f32 * PI / 180.0).cos() * projectile_max_speed
        ];

        manager::create_entity(AllEntityTypesEnum::Projectile, None, projectile_position, angle, Some(|entity: EntityTypeEnum| {
            let mut projectile = match entity {
                EntityTypeEnum::Projectile(entity) => entity,
                _ => unreachable!()
            };

            projectile.generic_entity.model = "RocketProjectile".to_owned();
            projectile.projectile_type = ProjectileTypeEnum::RocketTower;
            projectile.death_tick = tick_number + projectile_lifetime_ticks;
            projectile.velocity = projectile_velocity;
            projectile.acceleration = Some(projectile_low_speed);
            ...
            projectile.aoe_range = building_info.projectile_aoe_range[tier_index_usize];
...
impl EntityTrait for RocketTower {
    fn on_die(&self) {
