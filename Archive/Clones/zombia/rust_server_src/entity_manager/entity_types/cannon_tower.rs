...
impl BuildingTrait for CannonTower {
    fn new(uid: u16, position: super::generic_entity::Position, rotation: u16) -> Self {
        let mut entity = CannonTower {
            ranged_building: AttackingBuilding {
                aiming_yaw: 0,
                firing_tick: 0,
                range_cells: Vec::new(),
                base_building: Building::new(uid, position, rotation, "CannonTower".to_owned()),
...
impl AttackingBuildingTrait for CannonTower {
    fn update_range_cells(&self) -> Option<Vec<Option<Position>>> {    
        let _tick_number = CONFIG.with(|c| c.borrow().tick_number);
    ...
    fn fire_ranged(&self, enemy_uid: u16) {
        ...
        let building_info = buildings::get_building("CannonTower");
        ...
        manager::create_entity(AllEntityTypesEnum::Projectile, None, this_position, angle, Some(|entity: EntityTypeEnum| {
            ...
            projectile.generic_entity.model = "CannonProjectile".to_owned();
...
impl EntityTrait for CannonTower {
    fn on_die(&self) {
