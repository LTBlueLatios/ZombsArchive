    ...
    try {
        connections.forEach((e) => {
            if (e.type == "user") {
                e &&
                    e.send(
                        encode(`sessions,  ;${JSON.stringify(sessionsNames)}`),
                    );
            }
        });
    } catch (e) {
    ...
    ws.on("message", (m) => {
        try {
            ...
            if (
                ...
                x[0] == 1 &&
                ws.isVerified &&
                sessions_1[ws.sessionConnectedToId]
            ) {
                x = x.slice(1);
                const opcode = x[0];
                if (opcode == 9) {
                    const data =
                        sessions_1[ws.sessionConnectedToId].codec.decode(x);
                    if (data.name == "BuyItem" && data.response.tier == 1) {
                        if (
                            data.response.itemName == "PetCARL" ||
                            data.response.itemName == "PetMiner"
                        )
                            return;
                        if (
                            data.response.itemName == "Pickaxe" &&
                            sessions_1[ws.sessionConnectedToId].inventory
                                .Pickaxe
                        )
                            return;
                        if (
                            data.response.itemName == "Spear" &&
                            sessions_1[ws.sessionConnectedToId].inventory.Spear
                        )
                            return;
                        if (
                            data.response.itemName == "Bow" &&
                            sessions_1[ws.sessionConnectedToId].inventory.Bow
                        )
                            return;
                        if (
                            data.response.itemName == "Bomb" &&
                            sessions_1[ws.sessionConnectedToId].inventory.Bomb
                        )
                            return;
                    }
                    if (
                        data.name == "SetPartyName" &&
                        !(
                            new TextEncoder().encode(data.response.partyName)
                                .length <= 49
                        )
                    )
                        return;
                    if (
                        data.name == "SendChatMessage" &&
                        !(
                            new TextEncoder().encode(data.response.message)
                                .length <= 249
                        )
                    )
                        return;
            ...
            if (hasAccess && x[0] == 2 && ws.isVerified) {
                sessions_1[ws.sessionConnectedToId] &&
                    sessions_1[ws.sessionConnectedToId].ws &&
                    sessions_1[ws.sessionConnectedToId].ws.readyState == 1 &&
                    sessions_1[ws.sessionConnectedToId].ws.send(x.slice(1));
                return;
            }
            let msg = decode(m);
            if (!hasAccess) {
            ...
            if (msg == "getleaderboarddata") {
                sendMessage(
                    `leaderboarddata,  ;${JSON.stringify(leaderboardData)}`,
                );
            }
            if (!hasAccess) return;
            if (msg == "user") {
            ...
            if (msg == "getverifiedrecords") {
                sendMessage(
                    `verifiedrecords,  ;${JSON.stringify(verifyRecords)}`,
                );
            }
            if (ws.type == "user") {
                if (msg.startsWith("verify")) {
                    ...
                    if (sessions[sid]) {
                        if (
                            sessions[ws.sessionConnectedToId] &&
                            sessions[ws.sessionConnectedToId][ws.id]
                        )
                        ...
                        sendSessions();
                        sessions[sid] &&
                            Object.values(sessions[sid]).forEach((e) => {
                                let ws = connections.get(e);
                                ws &&
                                    !ws.isVerified &&
                                    (ws.send(
                                        encode(
                                            `verifydata,  ;${JSON.stringify(sessions_1[sid].getSyncNeeds())}`,
                                        ),
                                    ),
                                    (ws.isVerified = true));
                            });
                ...
                if (
                    ...
                    if (opcode == 9) {
                        if (data.name == "BuyItem" && data.tier == 1) {
                            if (
                                data.itemName == "PetCARL" ||
                                data.itemName == "PetMiner"
                            )
                                return;
                            if (
                        ...
                        if (
                            data.name == "SetPartyName" &&
                            !(
                                new TextEncoder().encode(data.partyName)
                                    .length <= 49
                            )
                        )
                            return;
                        if (
                            data.name == "SendChatMessage" &&
                            !(
                                new TextEncoder().encode(data.message).length <=
                                249
                            )
                        )
                            return;
                    }
                    sessions_1[ws.sessionConnectedToId].sendPacket(
                        opcode,
                        data,
                    );
                }
                if (msg.startsWith("buffer") && ws.isVerified) {
                    ...
                    sessions_1[ws.sessionConnectedToId] &&
                        sessions_1[ws.sessionConnectedToId].ws &&
                        sessions_1[ws.sessionConnectedToId].ws.readyState ==
                            1 &&
                        sessions_1[ws.sessionConnectedToId].ws.send(
                            new Uint8Array(JSON.parse(args[1])),
                        );
                }
                ...
                if (msg.startsWith("ear")) {
                    sessions_1[ws.sessionConnectedToId] &&
                        (sessions_1[
                            ws.sessionConnectedToId
                        ].scripts.autorespawn = true);
                }
                if (msg.startsWith("dar")) {
                    sessions_1[ws.sessionConnectedToId] &&
                        (sessions_1[
                            ws.sessionConnectedToId
                        ].scripts.autorespawn = false);
                }
                if (msg.startsWith("eah")) {
                    sessions_1[ws.sessionConnectedToId] &&
                        (sessions_1[ws.sessionConnectedToId].scripts.autoheal =
                            true);
                }
                ...
                if (msg.startsWith("eab")) {
                    if (
                        !sessions_1[ws.sessionConnectedToId] ||
                        !sessions_1[ws.sessionConnectedToId].gs
                    )
                        return;
                    const _this = sessions_1[ws.sessionConnectedToId];
                    _this.scripts.autobuild = true;
                    _this.inactiveRebuilder.forEach((e, t) =>
                        _this.inactiveRebuilder.delete(t),
                    );
                    _this.rebuilder.forEach((e, t) =>
                        _this.rebuilder.delete(t),
                    );
                    Object.values(_this.buildings).forEach((e) => {
                        _this.rebuilder.set(
                            (e.x - _this.gs.x) / 24 +
                                ((e.y - _this.gs.y) / 24) * 1000,
                            [
                                (e.x - _this.gs.x) / 24,
                                (e.y - _this.gs.y) / 24,
                                e.type,
                ...
                if (msg.startsWith("eau")) {
                    ...
                    _this.inactiveReupgrader.forEach((e, t) =>
                        _this.inactiveReupgrader.delete(t),
                    );
                    _this.reupgrader.forEach((e, t) =>
                        _this.reupgrader.delete(t),
                    );
                    Object.values(_this.buildings).forEach((e) => {
                        _this.reupgrader.set(
                            (e.x - _this.gs.x) / 24 +
                                ((e.y - _this.gs.y) / 24) * 1000,
                            [
                                (e.x - _this.gs.x) / 24,
                ...
                if (msg == "eaa") {
                    sessions_1[ws.sessionConnectedToId] &&
                        (sessions_1[ws.sessionConnectedToId].scripts.autoaim =
                            true);
                }
                if (msg == "daa") {
                    sessions_1[ws.sessionConnectedToId] &&
                ...
                if (msg.startsWith("eatb")) {
                    sessions_1[ws.sessionConnectedToId] &&
                        (sessions_1[ws.sessionConnectedToId].scripts.autobow =
                            true);
                }
                ...
                if (msg.startsWith("eapr")) {
                    sessions_1[ws.sessionConnectedToId] &&
                        (sessions_1[
                            ws.sessionConnectedToId
                        ].scripts.autopetrevive = true);
                }
                if (msg.startsWith("dapr")) {
                    sessions_1[ws.sessionConnectedToId] &&
                        (sessions_1[
                            ws.sessionConnectedToId
                        ].scripts.autopetrevive = false);
                }
                if (msg.startsWith("eaph")) {
                    sessions_1[ws.sessionConnectedToId] &&
                        (sessions_1[
                            ws.sessionConnectedToId
                        ].scripts.autopetheal = true);
                }
                if (msg.startsWith("daph")) {
                    sessions_1[ws.sessionConnectedToId] &&
                        (sessions_1[
                            ws.sessionConnectedToId
                        ].scripts.autopetheal = false);
                }
                if (msg == "ept") {
                    if (!sessions_1[ws.sessionConnectedToId]) return;
                    const _this = sessions_1[ws.sessionConnectedToId];
                    ...
                    _this.playerTrickPsk =
                        !args[1] || args[1].length != 20
                            ? _this.partyShareKey
                            : args[1];
                }
                if (msg == "dpt") {
                ...
                if (msg == "drpt") {
                    sessions_1[ws.sessionConnectedToId] &&
                        (sessions_1[
                            ws.sessionConnectedToId
                        ].scripts.reverseplayertrick = false);
                }
                if (msg == "eaaz") {
                    sessions_1[ws.sessionConnectedToId] &&
                        (sessions_1[
                            ws.sessionConnectedToId
                        ].scripts.autoaimonzombies = true);
                }
                if (msg == "daaz") {
                    sessions_1[ws.sessionConnectedToId] &&
                        (sessions_1[
                            ws.sessionConnectedToId
                        ].scripts.autoaimonzombies = false);
                }
                if (msg == "eahrc") {
                    sessions_1[ws.sessionConnectedToId] &&
                        (sessions_1[ws.sessionConnectedToId].scripts.ahrc =
                            true);
                }
                if (msg == "dahrc") {
                    sessions_1[ws.sessionConnectedToId] &&
                ...
                if (msg == "earf") {
                    sessions_1[ws.sessionConnectedToId] &&
                        (serverMap.get(
                            sessions_1[ws.sessionConnectedToId].serverId,
                        ).filler = true);
                }
                if (msg == "darf") {
                    sessions_1[ws.sessionConnectedToId] &&
                ...
                if (msg == "epl") {
                    sessions_1[ws.sessionConnectedToId] &&
                        (sessions_1[
                            ws.sessionConnectedToId
                        ].scripts.positionlock = true);
                }
                if (msg == "dpl") {
                    sessions_1[ws.sessionConnectedToId] &&
                        (sessions_1[
                            ws.sessionConnectedToId
                        ].scripts.positionlock = false);
                }
                if (msg == "eape") {
                    sessions_1[ws.sessionConnectedToId] &&
                        (sessions_1[
                            ws.sessionConnectedToId
                        ].scripts.autopetevolve = true);
                }
                if (msg == "dape") {
                    sessions_1[ws.sessionConnectedToId] &&
                        (sessions_1[
                            ws.sessionConnectedToId
                        ].scripts.autopetevolve = false);
                }
                if (msg == "earc") {
                    sessions_1[ws.sessionConnectedToId] &&
                        (sessions_1[
                            ws.sessionConnectedToId
                        ].scripts.autoreconnect = true);
                }
                if (msg == "darc") {
                    sessions_1[ws.sessionConnectedToId] &&
                        (sessions_1[
                            ws.sessionConnectedToId
                        ].scripts.autoreconnect = false);
                }
                if (msg.startsWith("lock")) {
                    ...
                    if (args[1]) {
                        ...
                        _this.lockPos = {
                            x: x || _this.myPlayer.position.x,
                            y: y || _this.myPlayer.position.y,
                        };
                    } else {
                        _this.lockPos = {
                            x: _this.myPlayer.position.x,
                            y: _this.myPlayer.position.y,
                        };
                    }
                }
                ...
                if (msg.startsWith("changesessionname")) {
                    let args = msg.split(",  ;");
                    sessionsNames[args[1]] &&
                        (sessionsNames[args[1]].sessionName =
                            (args[2] && args[2].slice(0, 25)) || "Session");
                    sendSessions();
                }
                if (msg.startsWith("changesessionid")) {
                    let args = msg.split(",  ;");
                    sessionsNames[args[1]] &&
                        (sessionsNames[args[1]].sessionUserId = parseInt(
                            args[2],
                        ));
                    sendSessions();
                }
...
class Bot {
    constructor(sessionName = null, name = " ", sid = "", psk = "") {
        ...
        this.psk = psk;
        this.ws = new WebSocket(`wss://${this.host}:443/`, {
    ...
    sendData(data) {
        sessions[this.userId] &&
            Object.values(sessions[this.userId]).forEach((e) => {
                let ws = connections.get(e);
    ...
    async onMessage(msg) {
        ...
        if (opcode == 5) {
            try {
                wasmModule(
                    (e) => {
                        if (
                            new Uint8Array(e[5].extra)[0] == 0 &&
                            new Uint8Array(e[5].extra)[1] == 0 &&
                            new Uint8Array(e[5].extra)[2] == 0
                        ) {
                            new Bot(
                                this.sessionName,
                                this.name,
        ...
        if (opcode == 10) {
            this.sendPacket(10, {
                extra: codec.decode(new Uint8Array(msg.data), this.Module)
                    .extra,
            });
            return;
        }
        ...
        switch (opcode) {
            case 0:
                ...
                !this.hasVerified &&
                    ((this.userId = counts++),
                    (sessions[this.userId] = {}),
                    (sessionsNames[this.userId] = {
                        sessionName: this.sessionName || "Session",
    ...
    onEntitiesUpdateHandler(data) {
        ...
        data.entities.forEach((entity, uid) => {
            ...
            !entity_1
                ? this.entities.set(uid, {
                      uid: uid,
                      targetTick: entity,
                      model: entity.model,
                  })
                : Object.keys(entity).forEach(
                      (e) => (entity_1.targetTick[e] = entity[e]),
                  );
        });
        this.nearestPlayer = null;
        this.nearestZombie = null;
            ...
            if (this.scripts.autoaim) {
                if (
                    ...
                    const distance = Math.hypot(
                        entity.targetTick.position.x - this.myPlayer.position.x,
                        entity.targetTick.position.y - this.myPlayer.position.y,
                    );
        ...
        this.myPlayer =
            this.entities.get(this.uid) &&
            this.entities.get(this.uid).targetTick;
        this.myPet =
            this.myPlayer &&
            this.entities.get(this.myPlayer.petUid) &&
            this.entities.get(this.myPlayer.petUid).targetTick;
        const userCount = !!Object.keys(sessions[this.userId] || {}).length;
        if (!userCount && this.myPlayer) {
            this.scripts.autorespawn &&
                this.myPlayer.dead &&
                this.sendPacket(3, { respawn: 1 });
            this.scripts.autoheal &&
                this.myPlayer.health / 5 <= this.scripts.healset &&
                this.myPlayer.health > 0 &&
                this.heal();
        }
        if ((!userCount || this.scripts.autopetheal) && this.myPet) {
            this.scripts.autoheal &&
                (this.myPet.health / this.myPet.maxHealth) * 100 <=
                    this.scripts.pethealset &&
                this.myPet.health > 0 &&
        ...
        this.gs &&
            this.scripts.autobuild &&
            this.inactiveRebuilder.forEach((e) => {
                const x = e[0] * 24 + this.gs.x;
                const y = e[1] * 24 + this.gs.y;
                if (
                    Math.abs(this.myPlayer.position.x - x) < 576 &&
                    Math.abs(this.myPlayer.position.y - y) < 576
                ) {
                    this.sendPacket(9, {
                        name: "MakeBuilding",
                        x: x,
        ...
        this.gs &&
            this.scripts.autoupgrade &&
            this.inactiveReupgrader.forEach((e) => {
                const x = e[0] * 24 + this.gs.x;
                const y = e[1] * 24 + this.gs.y;
                if (
                    Math.hypot(
                        this.myPlayer.position.x - x,
                        this.myPlayer.position.y - y,
                    ) <= 768
                ) {
                    if (e[5] - this.tick <= 0) {
                        e[5] = this.tick + 7;
        ...
        this.scripts.autoaim &&
            this.nearestPlayer &&
            this.sendPacket(3, {
                mouseMoved:
                    ((Math.atan2(
                        this.nearestPlayer.position.y -
                            this.myPlayer.position.y,
                        this.nearestPlayer.position.x -
        ...
        this.scripts.autoaimonzombies &&
            this.nearestZombie &&
            this.sendPacket(3, {
                mouseMoved:
                    ((Math.atan2(
                        this.nearestZombie.position.y -
                            this.myPlayer.position.y,
                        this.nearestZombie.position.x -
                            this.myPlayer.position.x,
                    ) *
                        180) /
                        Math.PI +
        ...
        if (this.scripts.playertrick || this.scripts.reverseplayertrick) {
            ...
            if (!this.leaveOnce && daySeconds > 18) {
                this.leaveOnce = true;
                this.scripts.reverseplayertrick
                    ? this.sendPacket(9, {
                          name: "JoinPartyByShareKey",
                          partyShareKey: this.playerTrickPsk,
                      })
        ...
        if (
            server.filler &&
            this.tick > server.tick &&
            (this.players !== 32 || this.dayCycle.isDay)
        ) {
            let count = 0;
            ...
            if (count < 8) {
                ...
                let amtNeededToFill = this.dayCycle.isDay
                    ? 1
                    : 32 - this.players || 1;
        ...
        if (this.scripts.positionlock) {
            let x =
                (Math.round(
                    (((Math.atan2(
                        this.lockPos.y - this.myPlayer.position.y,
                        this.lockPos.x - this.myPlayer.position.x,
                    ) *
                        180) /
                        Math.PI +
                        450) %
            ...
            if (
                ...
                this.sendPacket(3, {
                    up: x == 0 || x == 45 || x == 315 ? 1 : 0,
                    down: x == 135 || x == 180 || x == 225 ? 1 : 0,
                    right: x == 45 || x == 90 || x == 135 ? 1 : 0,
                    left: x == 225 || x == 270 || x == 315 ? 1 : 0,
                });
                this.positionRest = x;
            } else {
    ...
    onEnterWorldHandler(data) {
        ...
        for (let i = 0; i < 26; i++)
            this.ws.send(
                new Uint8Array([
                    3, 17, 123, 34, 117, 112, 34, 58, 49, 44, 34, 100, 111, 119,
                    110, 34, 58, 48, 125,
                ]),
            );
        ...
        this.ws.send(
            new Uint8Array([
                9, 6, 0, 0, 0, 126, 8, 0, 0, 108, 27, 0, 0, 146, 23, 0, 0, 82,
                23, 0, 0, 8, 91, 11, 0, 8, 91, 11, 0, 0, 0, 0, 0, 32, 78, 0, 0,
                76, 79, 0, 0, 172, 38, 0, 0, 120, 155, 0, 0, 166, 39, 0, 0, 140,
                35, 0, 0, 36, 44, 0, 0, 213, 37, 0, 0, 100, 0, 0, 0, 120, 55, 0,
    ...
    onRpcUpdateHandler(data) {
        switch (data.name) {
                    ...
                    if (
                        ...
                        if (e.dead) {
                            ...
                            if (e.tier < _reupgrader[2]) {
                                !this.inactiveReupgrader.get(index) &&
                                    this.inactiveReupgrader.set(index, [
                                        _reupgrader[0],
                                        _reupgrader[1],
                                        _reupgrader[2],
                                        e.tier,
            ...
            case "SetItem":
                ...
                if (this.inventory.HatHorns && !this.inventory.HatHorns.stacks)
                    this.buy("HatHorns", 1);
                if (!this.inventory[data.response.itemName].stacks)
                    delete this.inventory[data.response.itemName];
                if (
                    data.response.itemName == "ZombieShield" &&
                    data.response.stacks
                )
                    this.equip("ZombieShield", data.response.tier);
                break;
            case "PartyInfo":
            ...
            case "DayCycle":
                ...
                if (!data.response.isDay) {
                    ...
                    this.joinOnce = false;
                    if (
                        this.scripts.autopetevolve &&
                        this.myPet &&
                        [9, 17, 25, 33, 49, 65, 97].includes(
                            Math.min(
                                Math.floor(this.myPet.experience / 100) + 1,
                                [9, 17, 25, 33, 49, 65, 97][
                                    this.myPet.tier - 1
                                ],
                            ),
                        ...
                        this.sendPacket(9, {
                            name: "BuyItem",
                            itemName: this.myPet.model,
                            tier: this.myPet.tier + 1,
                        });
                    }
                }
                break;
    ...
    getSyncNeeds() {
        ...
        syncNeeds.push({
            ...
            maxPlayers: 32,
            chatChannel: 0,
            effectiveDisplayName: this.entities.get(this.uid)
                ? this.entities.get(this.uid).targetTick.name
                : this.name,
            x1: 0,
            y1: 0,
            x2: 24000,
        ...
        syncNeeds.push({
            name: "PartyInfo",
            response: this.partyInfo,
            opcode: 9,
        });
        syncNeeds.push({
            name: "PartyShareKey",
            response: { partyShareKey: this.partyShareKey },
            opcode: 9,
        });
        syncNeeds.push({
            name: "DayCycle",
            response: this.dayCycle,
            opcode: 9,
        });
        syncNeeds.push({
            name: "Leaderboard",
            response: this.leaderboard,
            opcode: 9,
        });
        syncNeeds.push({
            name: "SetPartyList",
            response: Object.values(this.parties),
            opcode: 9,
        });
        ...
        return {
            ...
            isPaused: this.myPlayer ? this.myPlayer.isPaused : 0,
...
let e_ParameterType = JSON.parse(
    '{"0":"Uint32","1":"Int32","2":"Float","3":"String","4":"Uint64","5":"Int64","Uint32":0,"Int32":1,"Float":2,"String":3,"Uint64":4,"Int64":5}',
);
...
class BinCodec {
    ...
    decodeEntityUpdate(buffer) {
        ...
        for (let i = 0; i < brandNewEntityTypeCount; i++) {
            ...
            for (var j = 0; j < brandNewEntityCountForThisType; j++) {
                ...
                this.sortedUidsByType[brandNewEntityType].push(
                    brandNewEntityUid,
                );
            }
        }
        ...
        while (buffer.remaining()) {
            ...
            if (!(entityType in this.attributeMaps)) {
                throw new Error(
                    "Entity type is not in attribute map: " + entityType,
                );
            }
            let absentEntitiesFlagsLength = Math.floor(
                        ...
                        switch (attribute.type) {
                            ...
                            case e_AttributeType.Float:
                                player[attribute.name] =
                                    buffer.readInt32() / 100;
                                break;
                            case e_AttributeType.String:
                                player[attribute.name] =
                                    this.safeReadVString(buffer);
                                break;
    ...
    decodeBlendInternal(buffer, Module) {
        ...
        for (
            let firstSync = Module._MakeBlendField(228, buffer.remaining()),
                i = 0;
            buffer.remaining();
        ...
        for (
            var secondSync = Module._MakeBlendField(4, 152), i = 0;
            i < 64;
            i++
        ) {
    ...
    encodeBlend(buffer, item) {
        for (
            var e = new Uint8Array(item.extra), i = 0;
            i < item.extra.byteLength;
            i++
        )
...
let wasmBuffers;
fetch(
    "https://cdn.glitch.global/7d28c753-c81a-408d-9529-94f5b60a0e4d/zombs_wasm.wasm",
).then((e) =>
    e.arrayBuffer().then((r) => {
        wasmBuffers = r;
    }),
);
...
let wasmModule = (callback, data_12, hostname) => {
        ...
        for (; value < input; ) {
            ...
            if (128 & i) {
                ...
                if (192 != (224 & i)) {
                    ...
                    segmentedId =
                        segmentedId + String.fromCharCode(((31 & i) << 6) | b1);
                }
            } else {
                segmentedId = segmentedId + String.fromCharCode(i);
            }
    ...
    function _0x710b07$jscomp$0(text, value, key, code) {
        ...
        for (; i < text.length; ++i) {
            ...
            if (character >= 55296 && character <= 57343) {
                ...
                character =
                    (65536 + ((1023 & character) << 10)) | (1023 & _0x216e31);
            }
            if (character <= 127) {
                if (key >= c) {
                    break;
    ...
    function _0xaf9b5$jscomp$0(text) {
        ...
        for (; i < text.length; ++i) {
            ...
            if ($sendIcon >= 55296 && $sendIcon <= 57343) {
                $sendIcon =
                    (65536 + ((1023 & $sendIcon) << 10)) |
                    (1023 & text.charCodeAt(++i));
            }
            if ($sendIcon <= 127) {
                ++_0x41d111;
            } else {
                _0x41d111 =
                    _0x41d111 +
                    ($sendIcon <= 2047 ? 2 : $sendIcon <= 65535 ? 3 : 4);
            }
        }
        return _0x41d111;
    }
    function _0x45ab50$jscomp$0(untypedElevationArray) {
        _0x4f7d64$jscomp$0.HEAP8 = _0x43f8b2$jscomp$0 = new Int8Array(
            untypedElevationArray,
        );
        _0x4f7d64$jscomp$0.HEAP16 = _0x4204f0$jscomp$0 = new Int16Array(
            untypedElevationArray,
        );
        _0x4f7d64$jscomp$0.HEAP32 = _0x2917ec$jscomp$0 = new Int32Array(
            untypedElevationArray,
        );
        _0x4f7d64$jscomp$0.HEAPU8 = _0x2c159b$jscomp$0 = new Uint8Array(
            untypedElevationArray,
        );
        _0x4f7d64$jscomp$0.HEAPU16 = _0x37eff3$jscomp$0 = new Uint16Array(
            untypedElevationArray,
        );
        _0x4f7d64$jscomp$0.HEAPU32 = _0x3322a0$jscomp$0 = new Uint32Array(
            untypedElevationArray,
        );
        _0x4f7d64$jscomp$0.HEAPF32 = _0x28607a$jscomp$0 = new Float32Array(
            untypedElevationArray,
        );
        _0x4f7d64$jscomp$0.HEAPF64 = _0x241d97$jscomp$0 = new Float64Array(
            untypedElevationArray,
        );
    }
        ...
        function instantiate(id) {
            WebAssembly.instantiate(wasmBuffers, locals).then((fn) => {
                ...
                typeof callback == "function" &&
                    callback(
                        _0x4f7d64$jscomp$0.decodeOpcode5(hostname, data_12),
                    );
            });
        }
        var locals = {
    ...
    function _0x1cbea8$jscomp$0(_0xdcd74c$jscomp$0) {
        ...
        return (
            (!_0x1bcee7$jscomp$0.bufferSize ||
                ...
                (_0x1bcee7$jscomp$0.bufferSize = _0x5383b2$jscomp$0 + 1),
                (_0x1bcee7$jscomp$0.buffer = _0x141790$jscomp$0(
                    _0x1bcee7$jscomp$0.bufferSize,
                ))),
            _0x36268d$jscomp$0(
                _0x49bfc6$jscomp$0,
    ...
    _0x55729a$jscomp$0();
    var _0x33e8b7$jscomp$0 = (_0x4f7d64$jscomp$0.___wasm_call_ctors =
        function () {
            return (_0x33e8b7$jscomp$0 = _0x4f7d64$jscomp$0.___wasm_call_ctors =
                _0x4f7d64$jscomp$0.asm.h).apply(null, arguments);
        });
    var _0x6f9ca9$jscomp$0 = (_0x4f7d64$jscomp$0._main = function () {
        return (_0x6f9ca9$jscomp$0 = _0x4f7d64$jscomp$0._main =
            _0x4f7d64$jscomp$0.asm.i).apply(null, arguments);
    });
    var _0x1d0522$jscomp$0 = (_0x4f7d64$jscomp$0._MakeBlendField = function () {
        return (_0x1d0522$jscomp$0 = _0x4f7d64$jscomp$0._MakeBlendField =
            _0x4f7d64$jscomp$0.asm.j).apply(null, arguments);
    });
    var _0x141790$jscomp$0 = (_0x4f7d64$jscomp$0._malloc = function () {
        return (_0x141790$jscomp$0 = _0x4f7d64$jscomp$0._malloc =
            _0x4f7d64$jscomp$0.asm.l).apply(null, arguments);
    });
    var _0x620aa9$jscomp$0 = (_0x4f7d64$jscomp$0._free = function () {
        return (_0x620aa9$jscomp$0 = _0x4f7d64$jscomp$0._free =
            _0x4f7d64$jscomp$0.asm.m).apply(null, arguments);
    });
    _0x4f7d64$jscomp$0.decodeOpcode5 = function (hostname, extra) {
        _0x4f7d64$jscomp$0.hostname = hostname;
        let DecodedOpcode5 = codec.decode(
