...
class WebSocketHandler {
    ...
    createSocket(nameType = "fuckSeaWorld", name = "", preset = "") {
        ...
        initializeWorker(worker)
            .then(response => {
                ws.workerID = response;
                console.log(ws.workerID);
...
function fetchSocketbyID(id) {
    return wsHandler.sockets.find(ws => ws.workerID === id);
}

const worker = new Worker("worker-alts.js");
worker.onmessage = (event) => {
    ...
    if (status === "finalizeOpcode10") {
        const socket = fetchSocketbyID(workerID);
        socket.send(result);
    } else if (status === "opcode5Complete") {
        const socket = fetchSocketbyID(workerID);
        wsHandler.sendPacket(4, {
            displayName: socket.socketName,
            extra: extra
        }, "Singular", socket, true);
        socket.enterworld2data = enterworld2;
    }
};
...
function initializeWorker(worker) {
    return new Promise((resolve, reject) => {
        ...
        worker.postMessage({ action: "Create" });
    });
}
    ...
    if (game.world.inWorld && document.activeElement.tagName.toLowerCase() !== "input" && document.activeElement.tagName.toLowerCase() !== "textarea") {
        ...
        switch (event.code) {
            ...
            case "KeyN":
                ...
                wsHandler.sendPacket(9, { name: "BuyItem", itemName: "PetCARL", tier: 1 });
                wsHandler.sendPacket(9, { name: "EquipItem", itemName: "PetCARL", tier: 1 });
                break;
            case "KeyM":
                if (!states.controlled) return;
