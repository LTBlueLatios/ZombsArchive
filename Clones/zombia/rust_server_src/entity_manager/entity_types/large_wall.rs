...
impl LargeWall {
    pub fn set_property(&self, property_name: &str, value: Box<dyn Any>) {
        manager::with_entity(self.base_building.generic_entity.uid, |entity| {
            let EntityTypeEnum::LargeWall(entity) = entity else {
                panic!("Expected a LargeWall entity, but got a different type");
            };
...
impl BuildingTrait for LargeWall {
    fn new(uid: u16, position: super::generic_entity::Position, rotation: u16) -> Self {
        let entity = LargeWall {
            base_building: Building::new(uid, position, rotation, "LargeWall".to_owned())
        };
...
impl EntityTrait for LargeWall {
    fn on_die(&self) {
