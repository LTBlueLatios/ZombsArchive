...
class UiShopItem extends UiComponent {
    constructor(tool) {
        ...
        toolDiv.setAttribute("item-type", tool.class);
        super(toolDiv);
        ...
        if (tool.name == "ZombieShield") {
            ...
            this.element.innerHTML = `
            <img class="hud-shop-info-preview" src="./asset/images/Ui/Icons/${tool.class}/${tool.name}${tool.tiers > 0 ? "Tier1" : ""}.svg">
            <strong style="font-size:20px;margin-left: 5px;">${tool.name.split(/(?=[A-Z])/).join(" ")}</strong> <small class="hud-shop-item-tier" style="color: rgba(255, 255, 255, 0.4); font-size: 12px;">Tier 1</small>
            <div class="hud-shop-item-cost"></div>
            `;
        }
    ...
    update(tier = 0) {
        ...
        const maxTier = tier >= (this.toolData.tiers || 1);
        ...
        if (this.toolData.name == "ZombieShield") {
            ...
            this.previewElem.src = `./asset/images/Ui/Icons/${this.toolData.class}/${this.toolData.name}${this.toolData.tiers > 0 ? `Tier${tier == 0 ? 1 : nextTier}` : ""}.svg`;
        }
        if (this.toolData.tiers !== undefined) this.tierElem.textContent = tier == 0 ? "Not Purchased" : `Tier ${nextTier}`;
    ...
    updateResourceCosts() {
        const maxTier = this.tier >= (this.toolData.tiers || 1);
